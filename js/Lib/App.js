...
var App = {
    ...
    getPageUri: function(alias, args, parsed_args, path_only)
    {
        args = typeof args === 'undefined' ? {} : args;
        parsed_args = typeof parsed_args === 'undefined' ? null : parsed_args;
        path_only = typeof path_only === 'undefined' ? false : true;

        var alias_args = alias === '' ? [] : alias.split('/');

        var p_alias = '';
        for (var i = 0; i < alias_args.length; i++) {
            if (alias_args[i][0] !== ':') {
                p_alias += alias_args[i] + '/';
                continue;
            }

            var arg_name = alias_args[i].substring(1);
            if (!(arg_name in args)) {
                console.warn('Arg `%s` not set.', arg_name);
                p_alias += alias_args[i] + '/';
            ...
            p_alias += args[arg_name] + '/';

            if (parsed_args !== null)
                parsed_args[arg_name] = String(args[arg_name]);
        }

        if (path_only)
            return p_alias;
        else
            return this._config.getBase() + p_alias;
    },
    ...
    initialize: function(init_fns, ext_infos)
    {
        ...
        window.onpopstate = function() {
            self._parseUri(window.location.pathname);
        };
        this._parseUri(window.location.pathname);
    },
    ...
    setPage: function(page_name, args, push_state)
    {
        ...
        if (push_state) {
            window.history.pushState({}, self._currentPage.title,
                    uri);
            document.title = self._currentPage.title;
        } else {
            window.history.replaceState({}, self._currentPage.title,
                    uri);
            document.title = self._currentPage.title;
        }
    ...
    _parseUri: function(uri, push_state)
    {
        ...
        var base = base.substring(0, base.length);
        ...
        for (var page_name in pages) {
            ...
            for (var i = 0; i < alias_array.length; i++) {
                ...
                if (alias_array[i][0] === ':') {
                    ...
                    args[alias_array[i].substring(1)] = uri_array[i];
                    continue;
                }
